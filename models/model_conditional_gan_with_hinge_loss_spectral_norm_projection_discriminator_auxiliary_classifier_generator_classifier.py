...
class GAN:
    ...
    def build_model(self):
        ...
        fake_image, gen_class_logit = self.generator(t_z, t_real_caption)
        ...
        g_loss_dis_class = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=t_class_label, logits=disc_fake_class_logit))
        g_loss_gen_class = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=t_class_label, logits=gen_class_logit))
        g_loss = g_loss_image + g_loss_dis_class + g_loss_gen_class
        ...
        tf.summary.scalar("Generator loss : dis classifier", g_loss_dis_class)
        tf.summary.scalar("Generator loss : gen classifier", g_loss_gen_class)
