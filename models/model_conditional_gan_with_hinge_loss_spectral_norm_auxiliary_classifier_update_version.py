...
class GAN:
    ...
    def build_model(self):
        ...
        class_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=t_class_label, logits=disc_fake_class_logit)) + tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=t_class_label, logits=disc_real_class_logit))
        g_loss_image = -1 * tf.reduce_mean(disc_fake_image)
        g_loss = g_loss_image + class_loss
        ...
        d_loss = d_loss_image + class_loss
    ...
    def discriminator(self, image, t_text_embedding):
        ...
        with tf.variable_scope("discriminator", reuse=tf.AUTO_REUSE):
            ...
            h3_new = ops.lrelu( self.d_bn4(ops.conv2d_sn(h3_concat, self.options['df_dim']*8, 1,1,1,1, spectral_normed=True, update_collection=update_collection, name = 'd_h3_conv_new'))) #4
