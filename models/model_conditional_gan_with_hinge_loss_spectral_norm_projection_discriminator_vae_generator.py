...
class GAN:
    ...
    def build_model(self):
        ...
        code, mean, std = self.encoder(t_real_caption)
        reconstruct_code = self.decoder(code)
        code_reconstruct_loss = tf.reduce_mean(tf.square(t_real_caption - reconstruct_code))
        kl_divergence = 0.5 * tf.reduce_mean(tf.square(mean) + tf.square(std) - tf.log(1e-8 + tf.square(std)) -1)
        fake_image = self.generator(t_z, code)
        ...
        g_loss = -1 * tf.reduce_mean(disc_fake_image) + code_reconstruct_loss + kl_divergence
        ...
        tf.summary.scalar("Generator vae reconstruction loss", code_reconstruct_loss)
        tf.summary.scalar("Generator vae kl divergence", kl_divergence)
    ...
    def build_generator(self):
        ...
        code, _, _ = self.encoder(t_real_caption)
        fake_image = self.sampler(t_z, code)
