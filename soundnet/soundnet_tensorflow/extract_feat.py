...
def extract_feat(model, sound_input, config, filename):
    ...
    for idx in xrange(layer_min, layer_max):
        ...
        if config.is_save:
            ...
            # Modified by myself
            np.save(os.path.join(config.outpath, filename + ".npy"), np.mean(np.squeeze(feature), axis=0))
            print("Save layer {} with shape {} as {}".format(idx, np.mean(np.squeeze(feature), axis=0).shape, config.outpath + filename + ".npy"))
...
if __name__ == '__main__':
    ...
    with tf.Session(config=sess_config) as session:
        ...
        for i, sound_sample in enumerate(sound_samples):
            output = extract_feat(model, sound_sample, args, filename_list[i])
